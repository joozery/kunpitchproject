    ...
    <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 z-20 w-full max-w-7xl px-6">
      ...
      <div className="hidden md:grid grid-cols-4 gap-6">
        {items.map(({ id, label, Icon, color }) => (
          <button
            key={id}
            className="w-full flex items-center justify-center gap-3 rounded-full px-7 py-3.5 text-white shadow-lg backdrop-blur-sm"
            style={{ background: 'linear-gradient(135deg, rgba(15,23,42,0.95), rgba(30,41,59,0.95))' }}
          >
            <span className={`inline-flex items-center justify-center w-11 h-11 rounded-full ${color}`}>
              <Icon className="text-white w-6 h-6" />
            </span>
            <span className="font-oswald text-lg whitespace-nowrap">{label}</span>
          </button>
        ))}
      </div>

      {/* Mobile: horizontal scroll */}
      <div className="md:hidden flex gap-4 overflow-x-auto scrollbar-hide justify-start">
        {items.map(({ id, label, Icon, color }) => (
          <button
            key={id}
            className="min-w-[260px] flex items-center gap-3 rounded-full px-6 py-3 text-white shadow-lg backdrop-blur-sm"
            style={{ background: 'linear-gradient(135deg, rgba(15,23,42,0.95), rgba(30,41,59,0.95))' }}
          >
